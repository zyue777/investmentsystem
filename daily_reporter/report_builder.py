...
【VIX & 外汇】VIX水位+情绪1句。必须只汇报美元指数和离岸人民币最新具体报价（直接写具体数字，禁止写涨跌幅和方向）。

【隔夜国际头条】从"隔夜国际头条"数据中，选出最多3条对A股/全球市场影响最大的新闻，每条1行，必须翻译成中文输出。没有重大新闻则写"暂无重大事件"。

【中概ADR】5只涨跌+1句整体判断。

【宏观日历】从"宏观日历"数据中，只列出今日真正重要的事件（如重要国家的CPI、GDP、非农、PMI、利率决议等重磅经济数据，或召开重大会议）。时间+国家+事项一行。无重大事件则写"今日无重大事件"。普通数据坚决剔除。

【华尔街见闻】若"华尔街见闻早餐"字段有内容，提炼1-2条对A股/港股极具参考价值的增量信息（必须与前面的"隔夜国际头条"内容完全不同，严禁重复）。每条1句话。若无不同内容则写"暂无其他重点"。

【A股盘前关注】从"财联社快讯"和其他数据中，提炼1-2条最值得关注的信息。
...
⚠️异动说明（涨跌幅≥±3%的品种）：
- 有⚠️标注的品种，在其行末另起一行写：  → 原因：[从"近期商品资讯"中寻找原因，找到明确支撑则精简输出；若实在找不到明确提及，必须写【原因暂未明确】]
- 禁止脱离数据进行常识判断。
...
严格按以下格式输出，两个标签缺一不可。请以冷静、犀利的投研视角撰写，严禁使用冗长套话：

[REPORT]
📌 自选股日报 | {today[:10]}

⚠️ 指令覆盖规则：在自选股日报中，无需遵守上文“原因暂未明确”的强制标签规则。没有新闻支撑就直接写没有，不要凭空添加该标签。

（必须将以下每一只自选股作为一个版块单独列出，绝不能遗漏：）
● 股票名称(代码)  涨跌幅
  ➤ 公告：[一句话概括，无则写"无重大公告"]
（若有StockTwits讨论原文，在下方单独成行写出社区动态；若无有效讨论则直接省略该行，切勿写"暂无分析"）
  ➤ 社区：[必须提炼出原文中包含的**具体数据、具体的季节性规律、核心期权买卖或价格目标**等实质性细节。禁止空洞说“探讨了季节性”，必须说明“具体的逻辑是什么”。]

【科技产业脉动】（根据数据用1句话锋利地总结事件对AI产业链的影响，若是无关内容输出"今日暂无核心快讯"）
[/REPORT]
[DIARY]
...
【上午重大事件】从"重大财经新闻"、"财联社快讯"、"华尔街见闻"中，严格筛选出最多3条真正重大的宏观或行业事件（央行决策/重大经济数据/地缘政治/监管政策/重大并购）。无足够重大事件则写"暂无重大事件"。每条1行，不超过30字。
...
【华尔街见闻】从"华尔街见闻"数据中，挑出1-2条对A股/港股明日最有参考价值的国际资讯，一句话概括，不超过30字/条。无高参考价值则写"暂无特别关注项"。
