...
- [ ] `handle_confirm()` 中三种 type 的路径拼接正确：
  - `'new'` → `Path(kb) / '知识库' / '行业' / pending['path']`
  - `'phase'` → `Path(kb) / pending['path']`（相对路径，须加 kb 前缀）
  - `'update'` → `Path(pending['path'])`（框架文件存的是绝对路径）
- [ ] `_parse_record_output()` 剥除重复 `知识库/行业/` 前缀（`.replace('知识库/行业/', '')`）
- [ ] 框架更新（焦点+/图谱+/论+）的 `pending['path']` 存的是完整绝对路径

### DeepSeekBot
- [ ] URL feed 写入 `_pending_writes` 前已补全 `知识库/行业/` 前缀
- [ ] `write_kb_file()` 拼接路径：`KB_PATH / clean`（`clean` 已包含 `知识库/行业/`）
...
检查命令：
```bash
ls ~/桌面/投研工作台/ | grep -E "^inbox$|^draft$"
# 无输出 = 合规
```
...
2. 预期回复顺序：
   ...
   回复【确认】写入，回复【取消】放弃
3. 回复「确认」
4. 预期：✅ 已写入：/path/to/知识库/行业/[文件名].md
5. 验证文件确实存在（路径包含 知识库/行业/）
```
...
| 日期 | 问题 | 原因 | 修复 |
|------|------|------|------|
| 2026-04-11 | 全面迁移到新工作台 | 知识库重构为三层认知架构，目录结构全部重命名 | 删除 PHASE_MAP、替换全部硬编码路径、重写认知框架函数 |
...
| 2026-04-11 | DeepSeekBot 读取 `_Raw_Inbox` 旧文件并幻觉内容 | `list_kb_files()` 把临时文件暴露给 DeepSeek | 过滤 `_Inbox` 和 `inbox` 路径 |
| 2026-04-11 | 知识库根目录出现违规 `inbox/` | 两个 bot 把临时文件存入 `KB_ROOT/inbox/` | 改存 `_Inbox/` |
...
| 2026-04-11 | DeepSeekBot URL feed 情报卡写入知识库根目录散乱位置 | `_pending_writes['file_path']` 缺少 `知识库/行业/` 前缀 | 写入 pending 前自动补全前缀 |
...
- [ ] 写入路径包含正确的前缀（`知识库/行业/` 或 `研究/周报月报/` 等）
- [ ] 临时文件只写入 `_Inbox/`，不新建根目录下的临时文件夹
- [ ] 错误情况有明确的用户提示（`❌ ...`），不静默失败
